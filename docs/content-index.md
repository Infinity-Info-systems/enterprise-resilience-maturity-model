# Content Index

## Purpose

This index organizes the repo into a front door for enterprise resilience, governance maturity, and operational improvement.
Use it to move from the framework to the exact maturity or reporting artifact you need.
The order is designed to move from governance framing into the maturity artifacts and then into the reusable templates.

## Entry Points

- [Framework Overview](./framework-overview.md)
- [Resilience Governance](./resilience-governance.md)
- [Reliability Maturity](./reliability-maturity.md)
- [DR Maturity](./dr-maturity.md)
- [Observability Maturity](./observability-maturity.md)
- [FinOps Maturity](./finops-maturity.md)
- [Executive Resilience Reporting](./executive-resilience-reporting.md)

## Models and Scorecards

- [Reliability Governance Model](../models/reliability-governance-model.md)
- [Resilience Scoring Model](../models/resilience-scoring-model.md)
- [Enterprise Resilience Scorecard](../scorecards/enterprise-resilience-scorecard.md)
- [Platform Maturity Scorecard](../scorecards/platform-maturity-scorecard.md)

## Maturity Levels

- [Level 1: Initial](../maturity-levels/level-1-initial.md)
- [Level 2: Managed](../maturity-levels/level-2-managed.md)
- [Level 3: Defined](../maturity-levels/level-3-defined.md)
- [Level 4: Measured](../maturity-levels/level-4-measured.md)
- [Level 5: Optimized](../maturity-levels/level-5-optimized.md)

## Templates and Dashboards

- [Executive Roadmap Template](../templates/executive-roadmap-template.md)
- [Improvement Plan Template](../templates/improvement-plan-template.md)
- [Maturity Assessment Template](../templates/maturity-assessment-template.md)
- [Resilience Dashboard](../dashboards/resilience-dashboard.md)

## References

- [Bibliography](../references/bibliography.md)
- [Research Links](../publications/research-links.md)
- [Use-Case Notes](../evidence/use-case-notes.md)

## Reading Order

1. Framework overview
2. Resilience governance
3. Reliability maturity
4. DR maturity
5. Observability maturity
6. FinOps maturity
7. Executive resilience reporting

## Shared Direction

Use the same section structure as the other core repos so the ecosystem stays aligned and easy to navigate.
Keep the pages short enough for review and specific enough for action.
If a page does not help score, explain, or improve resilience, move that detail into a more specific artifact.
